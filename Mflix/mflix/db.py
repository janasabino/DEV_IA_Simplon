...
def get_db():
    ...
    MFLIX_DB_NAME = current_app.config["MFLIX_NS"] #"mflix" 
    if db is None:
...
def get_movies_by_country(countries):
    ...
    try:
        ...
        # and _id. Do not include a limit in your own iWrite a query that matches movies with the countries in the "countries"
        # list, but only returns the title and _id of each movie.mplementation, it is
        # included here to avoid sending 46000 documents down the wire.
        countries={"countries":{"$in": countries}}
        projection ={"_id":1,"title":1}
        return list(db.movies.find(countries,projection))
...
def get_movie(id):
    ...
    try:
        ...
        pipeline = [
            ...
            {
                # sort the comments in the $lookup stage
...
# 1. Click on “security” once you are in Clusters.
# 2. Create a custom role that will have readWrite acess to only the mflix database
# 3. Create a new user and assign that new role to that user.
# 4. Fix your connection string to connect with that user.
